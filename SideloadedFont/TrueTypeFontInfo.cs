...
public class TrueTypeFontInfo
{
    ...
    public static string FontFamily(Stream fontFileStream)
    {
        try
        {
            var analyzer = new TrueTypeFontInfo
            {
            ...
            {
                throw new Exception($"FontStream does not have valid version [{version}] value");
                //return string.Empty;
            }
        ...
        try
        {
            ...
            for (var i = 0; i < numTables; i++)
            {
                ...
                for (var record = 0; record < count; record++)
                {
                    ...
                    var str = System.Text.Encoding.UTF8.GetString(chars, 0, nameLength);
                    return str;
                }
            }
